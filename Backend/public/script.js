...
document.querySelectorAll('.nav-btn, .m-nav-item').forEach(btn => {  
    ...
    const presetTrigger = document.querySelector('.preset-trigger');  
    if (presetTrigger) {  
        presetTrigger.addEventListener('click', openPresetModal);  
    }  
...
function editTask(taskId) {  
    ...
    if (task) {  
        ...
        const priorityBtn = document.getElementById('priority-btn');  
        if (priorityBtn) {  
            priorityBtn.textContent = task.priority === 'high' ? 'Urgent 🔥' :   
                                    task.priority === 'medium' ? 'Normal Priority' : 'Low Priority';  
        }  
...
function selectPreset(seconds, label) {  
    ...
    // Update button label  
    const presetLabel = document.getElementById('active-preset-label');  
    if (presetLabel) {  
...
window.switchAccount = showSwitchAccountModal;  
